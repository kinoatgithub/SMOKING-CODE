...
/*休眠设置*/
#define KINO_TIME_TO_SLEEP_S			20										//设置无操作~秒后休眠

/*出厂功率校准设置窗口时间*/
#define KINO_ADJ_TEMP_TIME				2000									//上电后将有`的毫秒的时间可以触发高低温度设置
#define KINO_ADJ_BASE_POWER				( 378225UL )							//设计为当电池电压3.3V占空比为100%时的功率为基础值
...
#define KINO_LED_PWM_PERIOD				100     								//周期设置,每~个计数为一个pwm周期
...
/*ADC设置*/
#define KINO_ADC_DATE_VOLUME			32										//ADC数据缓冲区大小,只能是2的幂次方个，不能>64否则和数有溢出风险

/*按钮设置*/
#define KINO_BUT_BUF_NUM				4										//设置按钮历史事件时间点的容量，必须设为2^（int）x个
#define KINO_BUT_ACT_COUNT				3										//设置连续触发按钮~次为激活状态
#define KINO_BUT_ACT_MS					2000									//设置连续触发按钮在~毫秒内完成才激活，单位毫秒
#define KINO_BUT_DIS_ACT_MS				100										//两次触发时间间隔少于~毫秒，则合并为一次，单位毫秒，
...
#define KINO_USER_SMOKE_TIME_S			20										//恒功率时长单位秒
#define KINO_PRE_HEAT_TIME_MS			10000									//电热片预热时长单位毫秒
#define KINO_TIDE_HYSTERESIS_VOLUME		5										//恒功率模式电压历史纪录容量,连续~次电压单方向超量变化作为事件
